...
import {CategoryResponseType, CategoryType} from "../types/categories-response.type";
...
export class Budget {
    ...
    private async getOperations(): Promise<void> {
        const result: CategoryResponseType | DefaultResponseType | null = await HttpUtils.request('/operations?period=' + this.period);
        ...
        if ((result as CategoryResponseType).response) {
            this.showOperations((result as CategoryResponseType));
            // this.showOperations((result as CategoryResponseType).response);
        }
    }

    private showOperations(operations:CategoryResponseType): void {
        for (let i = 0; i < (operations.response as Array<CategoryType>).length; i++) {
            const trElement: HTMLTableRowElement | null = document.createElement('tr');
            if (trElement) {
                trElement.insertCell().innerText = (i + 1).toString();
                trElement.insertCell().innerHTML = CommonUtils.getOperationType((operations.response as Array<CategoryType>)[i].type);
                trElement.insertCell().innerText = (operations.response as Array<CategoryType>)[i].category;
                trElement.insertCell().innerText = (operations.response as Array<CategoryType>)[i].amount + '$';
                trElement.insertCell().innerText = new Date((operations.response as Array<CategoryType>)[i].date).toLocaleDateString('ru-RU');
                trElement.insertCell().innerText = (operations.response as Array<CategoryType>)[i].comment;
                trElement.insertCell().innerHTML =
                    '<button class="btn p-0 delete" type="button" data-bs-toggle="modal" data-bs-target="#modalSheet" id="' + (operations.response as Array<CategoryType>)[i].id + '">' +
                    '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">' +
                    '<path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/>' +
                    '<path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>' +
                    '</svg>' + '</button>' + '<a href="/#/edit?id=' + (operations.response as Array<CategoryType>)[i].id + '" class="btn p-0">' +
